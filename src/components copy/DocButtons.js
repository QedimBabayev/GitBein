    ...
    render() {
        ...
        const menu = (
            <Menu>
                ...
                <Menu.Item key="1">Qaytarma</Menu.Item>
            ...
            <div className='doc_header_buttons '>
                {
                    Object.values(this.props.buttonsName).filter(p => p.id === 'saveBtn').map(p =>
                        <Button onClick={this.handleClearChanged} form={p.form} htmlType={'submit'} loading={this.props.state.putdatas.fetchingSave} key={p.id} className={p.className} id={p.id}>{p.title}</Button>
                    )
                }
                {
                    Object.values(this.props.buttonsName).filter(p => p.id != 'saveBtn').map(p =>
